...
func TestValidWikiURL(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			url:    "https://feishu.cn/wiki/space/7543921285740380179",
			prefix: "https://feishu.cn",
			token:  "7543921285740380179",
			noErr:  true,
		},
		{
			name:   "validate wiki space url with query params",
			url:    "https://feishu.cn/wiki/space/7611058517562133435?ccm_open_type=lark_wiki_spaceLink&open_tab_from=wiki_home",
			prefix: "https://feishu.cn",
...
func TestParseFeishuUrl(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantPfx:  "https://feishu.cn",
			wantErr:  false,
		},
		{
			name:     "parse wiki space url with query params",
			url:      "https://feishu.cn/wiki/space/7543921285740380179?from=home",
			wantType: UrlTypeWikiSettings,
			wantTok:  "7543921285740380179",
			wantPfx:  "https://feishu.cn",
			wantErr:  false,
		...
		{
			name:     "parse file url",
			url:      "https://feishu.cn/file/T2fhb9qHoo406OxJT1yc600Fnyh",
