...
**引用跟随**：download/mirror 的 `--follow`（+`--follow-depth`，默认 1）把正文 @mention 与行内链接引用的 docx/wiki 文档 BFS 下载到 `<output>/_refs/`（`core/refs.go`、`core/follow.go`、`cmd/follow.go`）。parser 解析时旁路收集 `DocRef`（`Parser.RefDocs`），并随下载版本边车持久化（`download_manifest.go` 的 `DownloadRecord.Refs`）；skip-unchanged 路径直接回放记录中的 refs——**刻意不从 markdown 产物反解**（反解追不上渲染格式的所有细节：固定三反引号围栏对内容含 ``` 的代码块产出歧义 markdown、链接文本零转义可含 `]`、正文可撞评论附录 marker，漏一条 ref 就会让二次 mirror 的 prune 误删 `_refs/`）。旧版记录未采集 refs（`RefsRecorded=false`）时 `--follow` 视为过期、重下一次补录。树内去重：seen 同时记 NodeToken 与 ObjToken（飞书把 wiki @mention 解析为 ObjToken + obj-type=docx，URL 却仍是 wiki 形态）。正文链接零改写（round-trip 安全）；mirror 索引加「引用文档 (_refs)」节做 URL→本地路径映射，边车记录 follow 配置供无参重同步沿用；follow 失败 warn 继续且不阻止 prune（token 出队时已入 seen）
...
**本地文件链接（md2blocks/uploader）：** 正文行内链接指向本地存在的文件时分两类。`.md` 为**文档交叉引用**：读目标文件 frontmatter 的 `source` 转成指向对应飞书文档的普通链接（`resolveLocalMdLink`）；目标未上传则降级纯文本——块签名包含 link URL，待目标上传后再传本文档，增量 diff 会识别变更并原地 `update_text_elements` 补上链接。**多文件上传自动补链（pass 2）**：降级引用旁路收集进 `ConvertResult.UnresolvedMdRefs`（路径按 mdDir 展开、去重）、经 `UploadResult` 透出；pass 1 全部传完后 `planLinkRepairs`（`cmd/upload.go` 纯函数，绝对路径归一）选出「上传成功且降级引用目标也在本批次成功上传」的文件，自动二次**增量**上传补链（互引环、前向引用一次命令收敛）。刻意不做依赖图拓扑排序：环无解、且会打乱 wiki 节点创建顺序（参数顺序即知识库文档顺序）。仅多文件生效；`--dry-run` 只提示不执行（未落库、目标无 source）；补链失败仅警告 + `--json` 的 `link_repairs` 记录，不改退出码。其余扩展名为**附件**：生成空 file 块后传素材再 `replace_file`。file 块无法经 descendant API 创建（服务端要求 file 挂在自动生成的 View 块下，显式 `page→file` 树报 1770030 invalid parent children relation），创建请求也不接受 `name` 等字段（1770001）：`flushFlatBlocks` 在 file 块处切分批次，file 块单独走官方 children API（`client.CreateDocxFileBlock`，请求体为空 `file:{}`），从响应（或补查 view 子块）取真实 file 块 id 供素材上传与 `replace_file`——文件名由素材文件名决定。
