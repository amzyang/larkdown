...
- 打开开发配置/安全设置，重定向 URL 填写 `http://localhost:9999/callback`
...
| `--incremental` | `--incr` | false      | 增量更新（仅修改变化的块）   |
...
Skill 源码随仓库维护于 [`skills/larkdown/`](skills/larkdown/)（含 `SKILL.md` 与 `references/advanced-usage.md`）。在 Claude Code 中可将其软链或复制到 `~/.claude/skills/larkdown/`，或纳入插件 marketplace 后通过 `claude plugin install` 安装。
